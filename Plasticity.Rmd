...
#install.packages("fansi", type="binary")
...
```{r}
ggplot(BB_fitness_df3, aes(x=fl_duration, y=total_est_seed_production)) + geom_point(shape=1, size=1, col="darkgreen") + theme_minimal() + labs(title="Flower Duaration",x="Flower Duration", y = "Estimated Seed Production")
```
```{r}
...
```{r}
anova_fl_duration <- aov(fl_duration ~ Block, data = BB_fitness_df3)
summary(anova_fl_duration)
```
```{r}
TukeyHSD(anova_fl_duration)
```

```{r}
anova_FFD <- aov(FFD_DOY ~ Block, data = BB_fitness_df3)
summary(anova_FFD)
```
...
```
```{r}
hist(log(BB_fitness_df3$fl_duration+1))
```

```{r}
par(mfrow=c(2,2))
plot(anova_fl_duration)
```
```{r}

qqPlot(log(BB_fitness_df3$fl_duration+1))
```
```{r}
shapiro.test(log(BB_fitness_df3$fl_duration+1))
```
